...
export async function fetchInventory(steamid) {
  const url = `${BASE}/inventory/${steamid}/753/6?l=english&count=5000`;
...
export async function sellItem({ sessionid, assetid, priceNet, username }) {
  ...
  const res = await steamFetch(`${BASE}/market/sellitem/`, {
    method: 'POST',
    headers: {
      'Referer': `${BASE}/id/${username}/inventory/`,
...
export async function getGooValue({ sessionid, username, sourceAppid, assetid }) {
  const url = `${BASE}/id/${username}/ajaxgetgoovalue/`
...
export async function grindIntoGoo({ sessionid, username, sourceAppid, assetid, gooValueExpected }) {
  const fd = new FormData();
  fd.append('sessionid', sessionid);
  ...
  const res = await steamFetch(`${BASE}/id/${username}/ajaxgrindintogoo/`, {
    method: 'POST',
