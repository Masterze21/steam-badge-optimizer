...
export async function fetchHistogram(nameid) {
...
export async function createBuyOrder({ sessionid, mhn, priceTotal, quantity = 1, billing }) {
  const fd = new FormData();
  fd.append('sessionid', sessionid);
  ...
  // Champs de facturation (sans eux Steam retourne success:22).
  // On rejoue VERBATIM ce qui a été capturé, avec des valeurs par défaut sûres.
  const b = billing || {};
  const merged = {
    billing_address_two: '',
    billing_state: '',
  ...
  // Code postal : Steam selon les versions attend billing_po ou billing_postal_code
  // → on envoie les deux avec la même valeur (l'inconnue est ignorée).
  const pc = b.billing_po || b.billing_postal_code;
  if (pc) { merged.billing_po = pc; merged.billing_postal_code = pc; }
